...
{
	pthread_mutex_lock(&self-> mu_add_get);//This is the use of the global lock.
	while(self -> ReadersOnline > 0){//Here we do a loop if  readers exist.
		pthread_cond_wait(&self -> CondRead, &self -> mu_add_get);//Here  the readers wait for the writer to end.
	}
	self -> WritersOnline = self -> WritersOnline + 1;//The counter for the writers is increased by 1.
	...
	int AddReturn = memtable_add(self->memtable, key, value);//We create a variable for memtable to return it in the end of the function.
	self -> WritersOnline = self -> WritersOnline - 1;//The counter for the writers is decreased by 1.
	pthread_cond_broadcast(&self-> CondWriter);//Here we wake the readers.
	pthread_mutex_unlock(&self-> mu_add_get);//Here we unlock the global lock.
...
{
	pthread_mutex_lock(&self -> mu_add_get);//This is the use of the global lock.
	while(self-> WritersOnline > 0){//Here we do a loop if  writers exist.
		pthread_cond_wait(&self -> CondWriter, &self -> mu_add_get);//Here  the writers wait for the reader to read.
	}
	self->ReadersOnline = self->ReadersOnline + 1;//The counter for the readers is increased by 1.
	pthread_mutex_unlock(&self-> mu_add_get);//Here we unlock the global lock.
	int GetReturn = 0;//We create a integer that is zero.
    if (memtable_get(self->memtable->list, key, value) == 1){
        GetReturn = 1;//We give to the integer the value 1. 
	}
	if (GetReturn == 0){//In case that the above condition keeps the value 0   
		GetReturn = sst_get(self->sst, key, value);//Return this function
	}
	pthread_mutex_lock(&self-> mu_add_get);//This is the use of the global lock.
    self->ReadersOnline = self->ReadersOnline - 1;//The counter for the readers is decreased by 1.
    pthread_cond_signal(&self->CondRead);//Here we wake the writers.
    pthread_mutex_unlock(&self->mu_add_get);//Here we unlock the global lock.
    return GetReturn;
}
